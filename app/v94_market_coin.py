...
def register(app, base):
    ...
    def ensure():
        ...
        CREATE TABLE IF NOT EXISTS purple_coin_market(
          ...
          sell_pressure REAL NOT NULL DEFAULT 0, updated_at TEXT NOT NULL);
        ...
        ''');c.execute('INSERT OR IGNORE INTO purple_coin_market(id,price,prev_price,updated_at) VALUES(1,?,?,?)',(START_PRICE,START_PRICE,base.now_iso()));c.commit();c.close()
    ...
    def classify_trade(c,uid,t):
        ...
        recent=c.execute('SELECT symbol,side,total,executed_at FROM user_trades WHERE user_id=? AND id<? ORDER BY id DESC LIMIT 8',(uid,tid)).fetchall()
        same=[x for x in recent if x['symbol']==sym]
    ...
    def meaningful_count(c,uid):
        rows=c.execute('SELECT * FROM user_trades WHERE user_id=? ORDER BY id',(uid,)).fetchall()
        return sum(1 for t in rows if classify_trade(c,uid,t))

    def sync(uid):
        ensure();c=base.db()
        try:
            c.execute('BEGIN IMMEDIATE');c.execute('INSERT OR IGNORE INTO user_purple_currency(user_id) VALUES(?)',(uid,))
            w=c.execute('SELECT * FROM user_purple_currency WHERE user_id=?',(uid,)).fetchone();q=meaningful_count(c,uid);level=min(100,q//5);last=int(w['highest_rewarded_level'] or 0)
            s=c.execute('SELECT * FROM purple_currency_supply WHERE id=1').fetchone();remaining=max(0,int(s['max_supply'])-int(s['minted']));dr=c.execute('SELECT amount FROM purple_coin_daily_rewards WHERE user_id=? AND day=?',(uid,day())).fetchone();daily=int(dr['amount']) if dr else 0;minted=0;to=last
            for lv in range(last+1,level+1):
                reward=min(lv,MAX_PRICE and 100);reward=min(reward,remaining-minted,DAILY_MINT_CAP-daily-minted)
                if reward<=0:break
    ...
    def trade_coin(req:Order,request:Request):
        ...
        try:
            ...
            if req.side=='buy':
                ...
                c.execute('UPDATE user_accounts SET cash=cash-? WHERE user_id=?',(notional,uid));c.execute('UPDATE user_purple_currency SET balance=balance+? WHERE user_id=?',(qty,uid));pressure=qty
            else:
                if int(w['balance'])<qty:raise HTTPException(409,'Not enough Purple Coin')
                c.execute('UPDATE user_purple_currency SET balance=balance-? WHERE user_id=?',(qty,uid));c.execute('UPDATE user_accounts SET cash=cash+? WHERE user_id=?',(notional,uid));pressure=-qty
            circ=max(1,c.execute('SELECT minted-burned n FROM purple_currency_supply WHERE id=1').fetchone()['n']);impact=max(-.08,min(.08,pressure/max(100,circ)*.12));new=max(MIN_PRICE,min(MAX_PRICE,price*(1+impact)));c.execute('UPDATE purple_coin_market SET prev_price=price,price=?,volume=volume+?,buy_pressure=buy_pressure+?,sell_pressure=sell_pressure+?,updated_at=? WHERE id=1',(new,notional,qty if req.side=='buy' else 0,qty if req.side=='sell' else 0,base.now_iso()));c.execute('INSERT INTO purple_coin_price_history(price,volume,reason,created_at) VALUES(?,?,?,?)',(new,notional,req.side.upper(),base.now_iso()));c.execute('INSERT INTO purple_currency_ledger(user_id,delta,reason,level,trade_count,created_at) VALUES(?,?,?,?,?,?)',(uid,qty if req.side=='buy' else -qty,'SIM_MARKET_'+req.side.upper(),None,None,base.now_iso()));c.commit()
