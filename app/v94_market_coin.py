...
def register(app, base):
    ...
    def ensure():
        ...
        ''')
        try:
            c.execute('ALTER TABLE purple_coin_market ADD COLUMN exchange_reserve INTEGER NOT NULL DEFAULT 0')
        except Exception:
            pass
        c.execute('INSERT OR IGNORE INTO purple_currency_supply(id,max_supply,minted,burned) VALUES(1,?,0,0)',(HARD_CAP,))
        ...
        c.execute('INSERT OR IGNORE INTO purple_coin_exchange_reserve(id,balance,updated_at) VALUES(1,0,?)',(base.now_iso(),))
        s=c.execute('SELECT minted FROM purple_currency_supply WHERE id=1').fetchone()
        if s and int(s['minted'] or 0)>HARD_CAP:
            c.close()
            raise RuntimeError('Existing PPC minted supply exceeds the 10,000 PPC V9.9 cap; migration is required before startup.')
        c.execute('UPDATE purple_currency_supply SET max_supply=? WHERE id=1',(HARD_CAP,))
        c.commit();c.close()
    ...
    def reserve_balance(c):
        r=c.execute('SELECT balance FROM purple_coin_exchange_reserve WHERE id=1').fetchone()
