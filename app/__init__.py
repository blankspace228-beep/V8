...
def init_db_v82():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS admin_balance_audit (id INTEGER PRIMARY KEY AUTOINCREMENT,actor_user_id INTEGER NOT NULL,target_user_id INTEGER NOT NULL,old_cash REAL NOT NULL,new_cash REAL NOT NULL,reason TEXT,created_at TEXT NOT NULL)''')
    ...
    for r in rows:
        uid=r['id'];trades=conn.execute('SELECT COUNT(*) c FROM user_trades WHERE user_id=?',(uid,)).fetchone()['c'];positions=conn.execute('SELECT COUNT(*) c FROM user_positions WHERE user_id=?',(uid,)).fetchone()['c']
        if trades==0 and positions==0:conn.execute('UPDATE user_accounts SET cash=0,starting_cash=0,realized_pl=0 WHERE user_id=?',(uid,))
...
async def signup_v82(req:SignupV82,response:Response):
    ...
    salt,ph=base._new_password(req.password);code=f'{secrets.randbelow(1000000):06d}' if role!='owner' and EMAIL_VERIFICATION_REQUIRED else None;exp=(datetime.now(timezone.utc)+timedelta(minutes=15)).isoformat() if code else None
...
register_v93_currency(app,base)
