...
def register(app, base):
    ...
    def ensure():
        ...
        ''')
        conn.execute('INSERT OR IGNORE INTO purple_currency_supply(id,max_supply,minted,burned) VALUES(1,?,0,0)',(MAX_SUPPLY,))
        conn.commit();conn.close()

    def qualifying_trades(conn,uid):
        r=conn.execute('SELECT COUNT(*) c FROM user_trades WHERE user_id=? AND ABS(total)>=?',(uid,MIN_TRADE_NOTIONAL)).fetchone()
        return int(r['c'] or 0)
    ...
    def sync(uid):
        ...
        try:
            ...
            conn.execute('INSERT OR IGNORE INTO user_purple_currency(user_id) VALUES(?)',(uid,))
            wallet=conn.execute('SELECT * FROM user_purple_currency WHERE user_id=?',(uid,)).fetchone()
            trades=qualifying_trades(conn,uid);level=level_for(trades);last=int(wallet['highest_rewarded_level'] or 0)
            supply=conn.execute('SELECT * FROM purple_currency_supply WHERE id=1').fetchone();remaining=max(0,int(supply['max_supply'])-int(supply['minted']))
            minted_now=0;rewarded_to=last
            for lv in range(last+1,level+1):
                reward=min(reward_for(lv),remaining-minted_now)
                if reward<=0:break
                try:
                    conn.execute('INSERT INTO purple_currency_ledger(user_id,delta,reason,level,trade_count,created_at) VALUES(?,?,?,?,?,?)',(uid,reward,'LEVEL_REWARD',lv,trades,base.now_iso()))
                except Exception:
                    continue
                minted_now+=reward;rewarded_to=lv
            if minted_now:
                conn.execute('UPDATE user_purple_currency SET balance=balance+?,lifetime_earned=lifetime_earned+?,highest_rewarded_level=? WHERE user_id=?',(minted_now,minted_now,rewarded_to,uid))
                conn.execute('UPDATE purple_currency_supply SET minted=minted+? WHERE id=1',(minted_now,))
            conn.commit()
        except Exception:
            conn.rollback();raise
        finally:conn.close()
    ...
    def status(uid):
        ensure();conn=base.db();conn.execute('INSERT OR IGNORE INTO user_purple_currency(user_id) VALUES(?)',(uid,));conn.commit()
        w=conn.execute('SELECT * FROM user_purple_currency WHERE user_id=?',(uid,)).fetchone();s=conn.execute('SELECT * FROM purple_currency_supply WHERE id=1').fetchone();trades=qualifying_trades(conn,uid);level=level_for(trades)
        recent=[dict(x) for x in conn.execute('SELECT delta,reason,level,trade_count,created_at FROM purple_currency_ledger WHERE user_id=? ORDER BY id DESC LIMIT 12',(uid,)).fetchall()];conn.close()
        circulating=int(s['minted'])-int(s['burned'])
        return {'currency':CURRENCY,'symbol':SYMBOL,'balance':int(w['balance']),'level':level,'qualifying_trades':trades,'trades_to_next_level':0 if level>=MAX_LEVEL else TRADES_PER_LEVEL-(trades%TRADES_PER_LEVEL),'next_level_reward':0 if level>=MAX_LEVEL else reward_for(level+1),'highest_rewarded_level':int(w['highest_rewarded_level']),'supply':{'hard_cap':int(s['max_supply']),'minted_ever':int(s['minted']),'burned':int(s['burned']),'circulating':circulating,'unminted':max(0,int(s['max_supply'])-int(s['minted']))},'exchange':{'fake_cash_per_credit':CASH_PER_CREDIT,'redeemable_for_real_money':False},'starter_grant_available':not bool(w['starter_grant_claimed']),'recent_ledger':recent}

    class ExchangeReq(BaseModel): amount:int=Field(ge=1,le=10000)

    @app.get('/api/economy/purple-credit')
    def get_credit(request:Request):
        uid=base.current_user_id(request);return sync(uid)
